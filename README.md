...
```mermaid
flowchart LR
    A[Problema repetitivo] --> B[Entrada real]
    B --> C{Que tipo de caos?}
    C -->|print| D[OCR]
    C -->|planilha| E[Excel automation]
    C -->|evento| F[offline UI]
    C -->|grupo| G[web catalog]
    D --> H[Ferramenta local]
    E --> H
    F --> H
    G --> H
    H --> I[Teste com gente usando]
...
2026.06  foco em horafolio, OCR local, desktop apps e perfil publicado
...
[lapidando]     demos e documentação dos próximos projetos públicos
